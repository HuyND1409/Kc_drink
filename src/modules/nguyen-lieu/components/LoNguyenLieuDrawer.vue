<template>
  <a-drawer :open="open" :title="`📦 Lô hàng – ${nguyenLieu?.tenNguyenLieu ?? ''}`" width="850" @close="$emit('close')">
    ...
    <!-- Bảng lô hàng (FEFO) -->
    <a-table :columns="loColumns" :data-source="dsLo" :loading="loadingLo" :pagination="false" rowKey="idLo" bordered
      size="middle">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'soLuongTon'">
          <span style="font-weight: 600">{{ record.soLuongTon }}</span>
    ...
    <!-- Modal Nhập kho -->
    <a-modal :open="openNhapKho" title="Nhập kho nguyên liệu" ok-text="Nhập kho" cancel-text="Hủy"
      ...
      <a-form ref="nhapKhoFormRef" :model="nhapKhoForm" :rules="nhapKhoRules" layout="vertical">
        ...
        <a-form-item label="Nguyên liệu">
          <a-input :value="nguyenLieu?.tenNguyenLieu" disabled />
        </a-form-item>
        ...
        <a-form-item label="Số lượng nhập" name="soLuongTon">
          <a-input-number v-model:value="nhapKhoForm.soLuongTon" :min="1" placeholder="Nhập số lượng"
            style="width: 100%" :addon-after="nguyenLieu?.donViTinh" />
        </a-form-item>
...
const loColumns = computed(() => {
  ...
  if (isAdmin.value) {
    cols.push({ title: 'Thao tác', key: 'action', align: 'center' as const, width: 110 });
  }
  return cols;
});
...
const handleNhapKho = async () => {
  try {
    ...
    await createLoNguyenLieu({
      maLo: nhapKhoForm.maLo,
      idNguyenLieu: props.nguyenLieu!.idNguyenLieu,
